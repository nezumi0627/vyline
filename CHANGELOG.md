...
## [0.8.0-beta] — 2026-08-27 — アカウント設定・安全な引継ぎ・同期安定化

### 追加

- **Vyline Setup** — 初回ログイン後に表示する 3 ステップの設定画面を追加。進行状況を保存し、完了後は再表示しない。設定画面から再実行できる
- **MID ごとの設定** — 表示、通知、プライバシー、デバッグ、パフォーマンス、レイアウト設定をアカウントごとに保存。原子的な JSON 書き込みとスキーマバージョンを導入
- **安全な引継ぎ** — 設定のみを含む ZIP を作成・適用。manifest、ファイル SHA-256、匿名化 MID、上書き前バックアップを利用し、認証情報・トーク内容・鍵は含めない
- **診断ログ** — 個人情報をマスキングしたログの確認、JSON 出力、削除、GitHub Issue 作成画面への転記を追加
- **LINE ノート／アルバム操作** — backend の機能として公開

### 変更

- Windows では保存セッションを DPAPI（CurrentUser）で保護。サブデバイスはブラウザごとのランダムなインストール ID に結び付け、ハードウェア識別子は収集しない
- リモート利用は LAN 公開ではなく Tailscale を優先する方針へ更新。検出した Tailscale URL をバックエンドログへ表示
- 開いたチャット、既読抑止、未読位置などの UI 状態をアカウント単位で扱う

### 修正

- 未読のあるチャットは最初の未読位置、既読済みのチャットは末尾を開くように修正
- 仮想リストの計測後にスクロールすることで、初回表示が上部や不自然な位置で止まる問題を修正
- 既読送信後に古い未読数が再表示される問題を修正し、既読 API と同期ポーリングを安定化
