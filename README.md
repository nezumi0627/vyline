...
- **開発者の責任**: 本ソフトウェアの利用により生じた一切の問題（アカウント停止、データ破損、法的問題等）について、開発者は**一切の責任を負いません**。

## 著作権表示 / 出典表示

本ソフトウェアは **nezumi0627** (https://github.com/nezumi0627) 氏の手によって開発されています。
...
外部 `@evex/linejs` に依存せず、**自前のプロトコルスタック `@vyline/protocol`（Vyline）** で動作します。
...
- **利用規約・免責の同意ゲート**: ログイン直後に利用規約・免責事項を表示し、**同意しない限りアプリは一切動作しません**（同期・通信・表示を含む）。同意せずに本ソフトウェアを利用した場合、および想定しない手段（画面スキップ・設定改変・その他いかなる手法）で本画面を経ずに利用した場合も、**利用した時点で本規約に同意したものとみなし、開発者は一切の責任を負いません**
