...
やりたい機能
  ...
  → bun run nezu:find-native -- <名前>
  → Desktop の TalkService_*_pargs / line::Foo
  → nezuline domain + backend 実装
```

コード上の辞書: `Vyline/packages/nezuline/src/dictionary/rpcMap.ts`  
...
```powershell
# 文字列 + xref のみ（速い）
bun run nezu:find-native -- updateProfileAttributes --list-only --skip-setup

# decompile まで（Ghidra 必要・遅い）
bun run nezu:find-native -- sendMessage --max-functions 10

# Desktop インストール一式を source/desktop へ
bun run nezu:dump-desktop
bun run nezu:dump-desktop -- --full
