...
> ⚠️ **実装状況**: 暗号化ローカルバックアップの検出、履歴・メディアの抽出、Vyline DB への復元に対応しています。
...
> 🎬 **メディアについて**
>
> バックアップ内の参照情報と実ファイルを照合できた画像・動画・音声・ファイル・スタンプを復元します。端末側に実体が残っていない項目はスキップされます。
...
バックアップ内に実体があり、メッセージとの照合に成功したメディアは復元されます。端末側ですでに削除されたファイルや、参照情報を特定できないファイルはスキップされます。
...
- **抽出先**: `VYLINE_DATA_DIR/tmp/backup-work/ios/` 配下の作業領域。復号・抽出・索引・staging の合計を 10 GiB で打ち切り、履歴は正規化 SQLite staging に 500 件単位で投入します。処理後は作業領域を削除します
- **Manifest.plist**: 全体読み込みが必要なため既定 8 MiB で拒否します。特殊なバックアップだけ `VYLINE_IOS_MAX_MANIFEST_PLIST_BYTES` で上限をバイト指定できます
- **E2EE**: 復元後、グループ鍵等は自動で再取得・再導入されます（`ensureGroupE2EEKey` 経由）
- **メディア**: staging SQLite 内の参照キーと FTS5 抽出ファイル索引を照合し、実体をディスク間コピーで保存します。履歴 DB の取り込みに失敗した場合は、その復元で新規作成したメディアだけをロールバックします
