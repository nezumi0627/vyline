...
export function ChatArea() {
  ...
  const canCall = !!chat && canDirectCall(chat.id)
  ...
  const rows = useMemo<VirtualRow<MsgRow>[]>(() => {
    ...
    chatMessages.forEach((m, i) => {
      ...
      const sameAuthorAsNext = next && next.authorId === m.authorId && dayLabel(next.createdAt) === dl
      const sameAuthorAsPrev = prev && prev.authorId === m.authorId && dayLabel(prev.createdAt) === lastDay
      out.push({
        key: `msg-${m.id}`,
        item: {
          key: `msg-${m.id}`,
  ...
  if (!chat) {
    return (
      <div className="hidden flex-1 items-center justify-center bg-[var(--vy-chat-bg)] md:flex" data-pattern="0">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <p className="text-sm font-medium text-[var(--vy-text-dim)]">チャットを選択してください</p>
  ...
  const panelItems: MenuItem[] = [
    ...
    { label: "一番下へスクロール", icon: <IconArrowDown size={16} />, onClick: () => scrollToBottom("smooth") },
    {
      label: chat.muted ? "ミュートを解除" : "通知をミュート",
      icon: chat.muted ? <IconBell size={16} /> : <IconBellOff size={16} />,
      onClick: () => toggleMute(chat.id),
    },
    { label: "NezuTheme を開く", icon: <IconPalette size={16} />, onClick: () => setScreen("settings") },
    { label: "プロフィールを表示", icon: <IconMore size={16} />, onClick: () => setProfileDrawer(true) },
  ]
    ...
    <div className="flex h-full min-w-0 flex-1">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="flex items-center gap-2 border-b border-[var(--vy-border)] bg-[var(--vy-surface)] px-3 py-2.5 md:gap-3 md:pl-12 md:pr-4">
          ...
          >
            <Avatar glyph={streamerMode ? "•" : chat.avatar} color={chat.color} size={40} online={chat.online} imageUrl={streamerMode ? undefined : chat.avatarUrl} />
            <span className="min-w-0">
              <span className="flex items-center gap-1.5">
                <span className="vy-chat-header-title truncate text-sm font-semibold">{name}</span>
                {chat.isOfficial && <OfficialBadge />}
                {chat.muted && <IconBellOff size={13} className="shrink-0 text-[var(--vy-text-dim)]" />}
              </span>
              <span className="block truncate text-xs" style={{ color: chat.online ? "#3fd07d" : "var(--vy-text-dim)" }}>
                {chat.status}
              </span>
            </span>
          </button>
          ...
          <div className="vy-fade-in flex items-center gap-2 border-b border-[var(--vy-border)] bg-[var(--vy-surface)] px-3 py-2 md:px-4">
            <div className="flex flex-1 items-center gap-2 rounded-xl bg-[var(--vy-surface-2)] px-3 py-2">
              ...
              <span className="shrink-0 text-xs tabular-nums text-[var(--vy-text-dim)]">
                {search.q.trim() ? `${matches.length ? (search.index % matches.length) + 1 : 0}/${matches.length}` : ""}
              </span>
            </div>
            <button
              type="button"
              disabled={!matches.length}
              onClick={() => setSearch((s) => ({ ...s, index: (s.index - 1 + matches.length) % matches.length }))}
              aria-label="前の一致"
      ...
      {panel && <MessageContextMenu x={panel.x} y={panel.y} items={panelItems} onClose={() => setPanel(null)} />}
      {call && (
        <CallOverlay
          kind={call.kind}
          name={name}
