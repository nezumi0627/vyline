...
function recomputeSticonRanges(
  text: string,
  resources: SticonResource[],
): SticonResource[] {
  ...
  for (let i = 0; i < text.length; i++) {
    if (text[i] === STICON_PLACEHOLDER && ri < resources.length) {
      ...
      out.push({ productId: r.productId, sticonId: r.sticonId, S: i, E: 1 })
    }
  }
  return out
}

function syncSticonsToText(
  text: string,
  resources: SticonResource[],
): SticonResource[] {
...
export function MessageInput({ chatId }: { chatId: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void api.line.chatMembers(accountId, chatId).then((res: { ok: boolean; members?: Array<{ mid: string; displayName: string; thumbnailUrl?: string }> }) => {
      if (cancelled || !res.ok || !res.members) return
      const members = res.members.map((m: { mid: string; displayName: string; thumbnailUrl?: string }) => mapMember(m.mid, m.displayName, m.thumbnailUrl))
  ...
  function insertLineEmoji(packageId: string, sticonId: string) {
    ...
    const next = recomputeSticonRanges(
      useStore.getState().drafts[chatId] ?? "",
      [...prev.slice(0, before), { productId: packageId, sticonId }, ...prev.slice(before)],
    )
  ...
  function onDraftChange(value: string, caret?: number) {
    ...
    setDraftMentions(chatId, recomputeMentionsOnEdit(oldText, value, state.draftMentions[chatId] ?? []))
    const trig =
      chat?.type === "group" ? detectMentionTrigger(value, caret ?? value.length) : null
  ...
  function insertMention(opt: { mid?: string; all?: boolean; name: string }) {
    ...
    const end = mentionPicker ? start + 1 + mentionPicker.query.length : (ta?.selectionStart ?? text.length)
    ...
    >
      ...
      {replyMsg && (
        ...
        >
          ...
          >
            ...
            <p className="truncate text-sm text-[var(--vy-text-dim)]">{replyPreviewText(replyMsg)}</p>
          </button>
          <button
            type="button"
      ...
      {recording ? (
        <div className="vy-fade-in flex items-center gap-3 rounded-2xl border border-[var(--vy-border)] bg-[var(--vy-surface-2)] px-3 py-2.5">
          <span className="flex h-3 w-3 animate-pulse rounded-full" style={{ background: "var(--vy-danger)" }} aria-hidden />
