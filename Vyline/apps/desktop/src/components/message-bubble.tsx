...
import { useStore, formatTime, memberDisplayName, memberGlyph, type Message, type Chat } from "@/lib/store"
...
import { IconReply, IconCopy, IconTrash, IconCheck, IconPlay, IconChevron, IconDownload, IconAtSign } from "@/components/icons"
    ...
    <>
      {runs.map((r, i) =>
        r.type === "emoji" ? (
          <span
            key={i}
            className="inline-block text-[1.15em] leading-none align-[-0.05em]"
          >
            {r.value}
          </span>
        ) : (
...
function ReactionBadges({
  ...
  const entries = [...byType.entries()].filter(([t]) => REACTION_EMOJI[t])
  if (!entries.length) return null
  return (
    <div className={cn("mt-1 flex flex-wrap gap-1 px-1", side === "right" ? "justify-end" : "justify-start")}>
      {entries.map(([type, { count, mine }]) => (
        <button
          ...
          className={cn(
            "flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs transition-colors hover:bg-[var(--vy-surface-2)]",
            mine && "border-[var(--vy-accent)] bg-[color-mix(in_oklab,var(--vy-accent)_15%,transparent)]",
          )}
...
export const MessageBubble = memo(function MessageBubble({
  ...
  const react = (type: number, mine: boolean) => {
    ...
    const name = ({ 2: "NICE", 3: "LOVE", 4: "FUN", 5: "AMAZING", 6: "SAD", 7: "OMG" } as Record<number, string>)[type]
    if (!name) return
    ...
    void api.line.react(accountId, message.id, name as "NICE" | "LOVE" | "FUN" | "AMAZING" | "SAD" | "OMG").then((res) => {
      if (!res.ok) {
  ...
  const menuItems: MenuItem[] = [
    ...
    ...(!chat.isOfficial
      ? [
          { label: "👍 いいね", icon: <IconCheck size={16} />, onClick: () => react(2, false) },
          { label: "❤️ 愛してる", icon: <IconCheck size={16} />, onClick: () => react(3, false) },
          { label: "😆 面白い", icon: <IconCheck size={16} />, onClick: () => react(4, false) },
          { label: "🎉 すごい", icon: <IconCheck size={16} />, onClick: () => react(5, false) },
          { label: "😢 悲しい", icon: <IconCheck size={16} />, onClick: () => react(6, false) },
          { label: "😲 びっくり", icon: <IconCheck size={16} />, onClick: () => react(7, false) },
        ]
      : []),
                ...
                children: [
                  {
                    ...
                    icon: <IconDownload size={16} />,
                    onClick: () =>
                      downloadUrl(stickerAnimationUrl(message.sticker!), `sticker_${message.id}.png`),
                  },
                  {
                    label: "通常画像",
                    icon: <IconDownload size={16} />,
    ...
    ...((message.kind === "image" || message.kind === "video") && message.imageSrc
      ...
      : []),
    ...(!isMe && !message.read
      ? [{ label: "既読にする", icon: <IconCheck size={16} />, onClick: () => markRead(message.id) }]
      : []),
    ...(isMe && !message.revoked && message.status !== "sending" && !message.id.startsWith("pending_")
      ? [{ label: "送信を取り消し", icon: <IconTrash size={16} />, onClick: () => revokeMessage(message.id), danger: true }]
      : []),
  ]
  ...
  const readers =
    chat.type === "group" && message.readBy
      ? message.readBy.map((id) => ({
          id,
          name: memberDisplayName(chat.members?.find((m) => m.id === id)?.name ?? id, streamerMode),
        }))
      : []

  const canReaderList = isMe && chat.type === "group" && settings.showReaderList && readers.length > 0 && message.read
  ...
  const metaLine = !message.revoked && (
    <div className={cn("mt-1 flex items-center gap-2 px-1 text-[0.7rem] text-[var(--vy-text-dim)]", isMe ? "flex-row-reverse" : "flex-row")}>
      ...
      {canReaderList && (
        ...
        >
          既読者
          <IconChevron size={12} className={cn("transition-transform", showReaders && "rotate-90")} />
        </button>
      )}
    </div>
  )
    ...
    <div className={cn("flex w-full gap-2 px-1", isMe ? "flex-row-reverse" : "flex-row")}>
      ...
      >
        ...
        {message.revoked ? (
          ...
          >
            {isMe ? "あなたがメッセージの送信を取り消しました" : "メッセージの送信が取り消されました"}
          </div>
          ...
          >
            ...
            {message.sticker && (message.sticker.startsWith("http") || message.sticker.startsWith("/api/")) ? (
              <img
                src={message.stickerAnimated ? stickerAnimationUrl(message.sticker) : message.sticker}
                alt="スタンプ"
                className={cn("h-32 w-32 object-contain", message.stickerSticky && "drop-shadow-md")}
                draggable={false}
              />
            ) : (
          ...
          <button type="button" {...pressHandlers} className="vy-pop-in cursor-default text-6xl leading-none" aria-label="絵文字">
            {replyQuote}
            {message.sticons?.length ? (
              <Highlighted text={message.text ?? ""} sticons={message.sticons} mentions={message.mentions} />
            ) : (
              message.text
            )}
          </button>
          ...
          <div {...pressHandlers} className="vy-msg-enter max-w-[min(100%,340px)]">
            {replyQuote}
            {message.flexJson ? (
              <FlexMessageView container={message.flexJson} altText={message.altText || message.text} />
            ) : (
      ...
      {menu && <MessageContextMenu x={menu.x} y={menu.y} items={menuItems} onClose={() => setMenu(null)} />}
