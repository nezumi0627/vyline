...
export interface AutoReplyConfig {
	...
	perChat: Record<string, { enabled?: boolean; message?: string }>;
...
export const api = {
	auth: {
		...
		switch_: (accountId: string) =>
			request<{
				...
				restored?: boolean;
				error?: string;
			}>("POST", `/auth/switch/${encodeURIComponent(accountId)}`),
	...
	line: {
		...
		unsend: (accountId: string, messageId: string) =>
			request<UnsendResponse>("POST", `/line/${accountId}/unsend`, {
				messageId,
			}),
		...
		vylineWarm: (accountId: string, mids: string[]) =>
			request<{
				...
				count?: number;
				error?: string;
			}>("POST", `/line/${accountId}/vyline/warm`, { mids }),
		...
		chatMembers: (accountId: string, chatMid: string) =>
			request<{
				...
				"GET",
				`/line/${accountId}/chats/${encodeURIComponent(chatMid)}/members`,
			),
		...
		updateProfileBackground: (
			...
			fetch(
				`${BASE}/line/${encodeURIComponent(accountId)}/profile/background`,
				{
					method: "POST",
					headers: { "Content-Type": mime },
					body: bytes,
		...
		getAutoReply: (accountId: string) =>
			request<{ ok: boolean; config?: AutoReplyConfig }>(
				"GET",
				`/line/${accountId}/autoreply`,
			),
		...
		setAutoReplyChatOverride: (
			accountId: string,
			chatMid: string,
			override: { enabled?: boolean; message?: string },
		) =>
			request<{ ok: boolean; config?: AutoReplyConfig }>(
				"PUT",
				`/line/${accountId}/autoreply/${chatMid}`,
				override,
			),

		clearAutoReplyChatOverride: (accountId: string, chatMid: string) =>
			request<{ ok: boolean; config?: AutoReplyConfig }>(
				"DELETE",
				`/line/${accountId}/autoreply/${chatMid}`,
			),

		resetReadCache: (accountId: string, chatMid?: string) =>
			request<{ ok: boolean; cleared?: number; error?: string }>(
				"POST",
				`/line/${accountId}/read/reset-cache`,
				chatMid ? { chatMid } : {},
			),

		listScheduledMessages: (accountId: string) =>
			request<{ ok: boolean; items?: ScheduledMessage[] }>(
				"GET",
				`/line/${accountId}/scheduled-messages`,
			),

		addScheduledMessage: (accountId: string, chatMid: string, text: string, sendAt: number) =>
			request<{ ok: boolean; item?: ScheduledMessage; error?: string }>(
				"POST",
				`/line/${accountId}/scheduled-messages`,
				{ chatMid, text, sendAt },
			),

		removeScheduledMessage: (accountId: string, id: string) =>
			request<{ ok: boolean; removed?: boolean }>(
				"DELETE",
				`/line/${accountId}/scheduled-messages/${id}`,
			),
		...
		setProxy: (accountId: string, enabled: boolean, url: string) =>
			request<{
				...
				error?: string;
			}>("PUT", `/line/${accountId}/proxy`, { enabled, url }),
		...
		backupCreate: (
			accountId: string,
			opts: { chatMids?: string[]; includeMedia?: boolean },
		) =>
			request<{
				ok: boolean;
				summary?: {
		...
		groupCallStatus: (accountId: string, chatMid: string) =>
			request<{
				...
				"GET",
				`/line/${accountId}/call/group-status?chatMid=${encodeURIComponent(chatMid)}`,
			),
		...
		liff: {
			warm: (
				accountId: string,
				app: "ladder" | "schedule" | "poll",
				chatMid: string,
			) =>
				request<{ ok: boolean }>("POST", `/line/${accountId}/liff/warm`, {
					app,
					chatMid,
				}),
		},
