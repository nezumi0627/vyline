...
export function parseSticonReplace(meta?: Record<string, string | undefined> | null): SticonResource[] {
  if (!meta?.REPLACE) return []
  try {
    const parsed = JSON.parse(meta.REPLACE) as SticonReplace
 ...
 */
export function segmentTextWithSticon(
  text: string,
