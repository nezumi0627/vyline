...
import { buildMembersFromMessages, looksLikeMid, mapChat, mapMember, mapMessage, type ContactInfo } from "./mappers.js";
import { lineCdnProxy, lineStickerUrl } from "../utils/lineMedia.js";
...
export type { Chat, ChatSort, Message, Member, NezuTheme, Settings, Screen } from "./store-types.js";
...
export const UPDATE_NOTES = {
  version: "0.4.0-beta",
  title: "Vyline 0.4.0-beta — Mention & Media",
  items: [
...
type State = {
  ...
  sendSticker: (chatId: string, packageId: string, stickerId: string, isPremium?: boolean) => Promise<void>;
  ...
  setTheme: (t: NezuTheme) => void;
  updateThemeField: (field: keyof NezuTheme, value: string | number) => void;
  ...
  refreshReadReceipts: (chatId: string) => Promise<void>;
...
export const useStore = create<State>()(
  persist(
    (set, get) => ({
      ...
      unlock: (pin) => {
        const { settings, seenUpdateVersion } = get();
        const nextScreen =
          seenUpdateVersion !== UPDATE_NOTES.version ? "home" : "chat";
      ...
      hydrateLineData: ({ profile, chats, messages, hiddenMids, contactCache }) => {
        ...
        const chatMessageFilter = (m: LineMessage) =>
          !m.to || m.to === chatId || m.from === chatId;
        const mappedMessages = chatId && accountId
          ? messages
              .filter(chatMessageFilter)
              .map((m) => mapMessage(m, chatId, accountId, contactCache))
          : [];

        const members = buildMembersFromMessages(
          messages.filter(chatMessageFilter),
          contactCache,
        );
        ...
        if (chatId) {
          ...
          if (mappedMessages.length > 0 && get().settings.readReceipts) {
      ...
      sendMessage: async (chatId, text, opts) => {
        ...
        try {
          ...
          optimisticMentions = parseMentions(opts?.contentMetadata as Record<string, string> | null);
        } catch {
          /* ignore */
        }
          ...
          try {
            ...
            if (res.message) {
              ...
              const finalMsg: Message = {
                ...mapped,
                kind: "sticker",
                sticker: mapped.sticker?.startsWith("http") || mapped.sticker?.startsWith("/api/")
                  ? mapped.sticker
            ...
            const existing = refreshDebounce.get(chatId);
            if (existing) clearTimeout(existing);
            refreshDebounce.set(chatId, setTimeout(() => {
              refreshDebounce.delete(chatId);
              void get().refreshMessages(chatId, { force: true });
            }, 800));
          } catch {
      ...
      sendLineEmoji: async (chatId, packageId, sticonId) => {
        ...
        const emojiSrc = lineCdnProxy(
          `https://stickershop.line-scdn.net/sticonshop/v1/sticon/${packageId}/android/${sticonId}.png`,
        );
        const optimistic: Message = {
          id: tempId,
          chatId,
        ...
        void (async () => {
          try {
            ...
            if (blob.size > 11_000_000) {
              window.alert(
                blob === file ? "ファイルが大きすぎます（11MB まで）" : "画像が大きすぎます（圧縮後も 11MB 超）",
              );
      ...
      revokeMessage: async (id) => {
        ...
        else if (!res.ok) window.alert(res.error ?? "取り消しに失敗しました");
      },
      ...
      retryMessage: async (id) => {
        ...
        try {
          ...
          if (confirmed) {
            ...
            set((st) => ({
              messages: st.messages.map((m) => (m.id === id ? { ...m, ...mapped, id: mapped.id } : m)),
              chats: st.chats.map((c) =>
                c.id === chatId
                  ? {
                      ...c,
      ...
      markChatRead: async (id) => {
        const { accountId, messages, settings } = get();
        set((st) => ({
          chats: st.chats.map((c) => (c.id === id ? { ...c, unread: 0 } : c)),
        }));
        if (!accountId || !settings.readReceipts) return;
        const last = [...messages]
          .reverse()
        ...
        await api.line.markAsRead(accountId, id, last.id).catch(() => {
      ...
      toggleHide: (id) =>
        set((st) => ({
          chats: st.chats.map((c) => (c.id === id ? { ...c, hidden: !c.hidden } : c)),
        })),
      toggleMute: (id) =>
        set((st) => ({
      ...
      refreshChatsSilently: async () => {
        ...
        try {
          ...
          if (res.ok && res.chats) {
            const hidden = new Set(get().chats.filter((c) => c.hidden).map((c) => c.id));
            const dismissed = getDismissedChatMids(accountId);
            set((st) => ({
              chats: res.chats!
                .filter((c) => !dismissed.has(c.mid))
                .map((c) => {
                ...
                return prev
                  ? {
                      ...
                      unread:
                        st.activeChatId === c.mid ? 0 : (c.unreadCount ?? prev.unread),
                      lastMessagePreview:
            ...
            const candidates = res.chats
              ...
              .sort(
                (a, b) =>
                  (b.mid === activeChatId ? 1 : 0) - (a.mid === activeChatId ? 1 : 0),
              );
            for (const c of candidates.slice(0, 3)) {
              void get().backfillChat(c.mid).catch(() => undefined);
            }
          }
        } catch {
          /* silent */
          ...
          if (res.ok && res.messages) {
            ...
            set((st) => {
              ...
              const keep = existingChat.filter((m) => {
                ...
                if (m.id.startsWith("pending_") || m.status === "sending" || m.status === "failed") {
                  // 同内容がサーバに載ったら捨てる
            ...
            if (get().settings.readReceipts && get().activeChatId === chatId) {
              void get().markChatRead(chatId);
      ...
      refreshReadReceipts: async (chatId) => {
        ...
        task = (async () => {
          ...
          for (const mid of allReaderMids) {
            ...
            void api.line.contactProfile(accountId, mid).then((pres) => {
              if (!pres.ok || !pres.profile) return;
              set((st) => ({
                chats: st.chats.map((c) => {
                  if (c.id !== chatId) return c;
                  const members = [...(c.members ?? [])];
                  const i = members.findIndex((x) => x.id === mid);
                  const name = pres.profile!.displayName;
                  const avatarUrl = pres.profile!.thumbnailUrl;
                  if (i >= 0) {
                    members[i] = {
                      ...members[i]!,
      ...
      mergeIncomingMessages: (chatId, incoming, opts) => {
        ...
        const existingIds = new Set(
          messages.filter((m) => m.chatId === chatId).map((m) => m.id),
        );
        ...
        const latest = fresh.length ? fresh.reduce((a, b) => (a.createdAt >= b.createdAt ? a : b)) : undefined;
          ...
          return {
            ...
            chats: st.chats.map((c) => {
              ...
              return {
                ...c,
                unread:
                  activeChatId === chatId || silent
                    ? c.unread
                    : c.unread + incomingFromPeer,
                ...(latest
                  ? {
      ...
      setMessageReaction: (messageId, reaction, myMid) => {
        const typeNum = ({ NICE: 2, LOVE: 3, FUN: 4, AMAZING: 5, SAD: 6, OMG: 7 } as Record<string, number>)[reaction];
      ...
      pollMessagesDelta: async (chatId) => {
        ...
        if (!accountId || !chatId) return;
        const now = Date.now();
        const lastAt = lastDeltaPollAt.get(accountId) ?? 0;
        if (now - lastAt < DELTA_POLL_MIN_MS) return;
        lastDeltaPollAt.set(accountId, now);
        ...
        if (Date.now() - started > 6_000) {
          lastDeltaPollAt.set(accountId, Date.now());
        }
      },
                ...
                for (const ev of res.events) {
                  if (ev.kind === "message") {
                    ...
                    if (get().settings.readReceipts) {
                      void get().refreshReadReceipts(ev.chatMid).catch(() => undefined);
                    }
                  }
                }
...
export function commonGroupsWith(
  chats: Chat[],
  memberId: string,
...
export function serializeTheme(t: NezuTheme): string {
