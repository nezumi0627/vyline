...
function makeSrt(): string {
  return captions.map(([title, caption], i) => {
...
async function main() {
  ...
  const server = Bun.spawn(["bun", "./node_modules/vite/bin/vite.js", "--host", "127.0.0.1", "--port", String(port)], { cwd: join(root, "Vyline", "apps", "desktop"), stdout: "ignore", stderr: "inherit" });
  ...
  try {
    ...
    for (let scene = 1; scene <= captions.length; scene += 1) {
      console.log(`[pr-video] scene ${scene}/${captions.length} (${Math.round((scene / captions.length) * 100)}%)`);
  ...
  const ffmpeg = Bun.spawn(["ffmpeg", "-y", "-i", webmPath, "-vf", "subtitles=recordings/vyline-pr-demo.srt", "-c:v", "libx264", "-pix_fmt", "yuv420p", mp4Path], { cwd: root, stdout: "ignore", stderr: "inherit" });
