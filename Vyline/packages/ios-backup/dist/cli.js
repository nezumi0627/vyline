...
function parseKeybag(backupKeyBag) {
  ...
  for (const [tag, data] of loopTLVBlocks(backupKeyBag)) {
    ...
    if (data.length === 4) {
      ...
      } else if (tagStr === "WRAP" && result2.wrap === 0) {
        result2.wrap = value;
      } else if (tagStr === "UUID" && result2.uuid.length === 0) {
        result2.uuid = data;
      } else {
        result2.attrs.set(tagStr, data);
      }
      continue;
    }
    if (tagStr === "UUID") {
      if (currentClassKey?.clas) {
        result2.classKeys.set(readUInt32BE(currentClassKey.clas, 0), currentClassKey);
      }
...
function unpack64bit(s) {
  ...
  let result2 = 0n;
  for (let i = 0;i < 8; i++) {
    result2 = result2 << 8n | BigInt(s[i]);
  }
  return result2;
}
function pack64bit(n) {
  const result2 = new Uint8Array(8);
  let value = n;
  for (let i = 7;i >= 0; i--) {
    result2[i] = Number(value & 0xffn);
    value >>= 8n;
  }
  return result2;
}
function aesUnwrap(kek, wrapped) {
  ...
  const result2 = new Uint8Array(n * 8);
  for (let i = 1;i <= n; i++) {
    result2.set(pack64bit(R[i]), (i - 1) * 8);
  }
  return result2;
}
...
function readUInt64BE(buf, offset) {
  let result2 = 0;
  for (let i = 0;i < 8; i++) {
    result2 = result2 << 8 | buf[offset + i];
  }
  return result2;
}
...
function readInt(data, offset, size) {
  let result2 = 0;
  for (let i = 0;i < size; i++) {
    result2 = result2 << 8 | data[offset + i];
  }
  return result2;
}
...
function readAsciiString(data, offset, info, offsets, objectRefSize, objects) {
  ...
  return new TextDecoder("ascii").decode(data.slice(dataOffset, dataOffset + length));
}
...
function readArray(data, offset, info, offsets, objectRefSize, objects) {
  ...
  for (let i = 0;i < count; i++) {
    ...
    result2.push(activeObjectResolver?.(ref) ?? objects[ref]);
  }
  return result2;
}
function readDict(data, offset, info, offsets, objectRefSize, objects) {
  ...
  for (let i = 0;i < count; i++) {
    const key = activeObjectResolver?.(keyRefs[i]) ?? objects[keyRefs[i]];
    result2[key] = activeObjectResolver?.(valueRefs[i]) ?? objects[valueRefs[i]];
  }
  return result2;
}
  ...
  try {
    ...
    for (let i = 0;i < lineFiles.length; i++) {
      ...
      try {
        ...
        const result2 = getFileDecryptedCopy(backup, manifestEntry, targetPath);
        const extracted = {
          fileID: row.fileID,
          domain: row.domain,
...
var BACKUP_ROOT = process.env["IOS_BACKUP_ROOT"] || join5(homedir(), "Apple", "MobileSync", "Backup");
var UDID = process.env["IOS_UDID"] || "00008140-000668921A82801C";
var PASSWORD = process.env["IOS_BACKUP_PASSWORD"] || process.argv[2];
var OUTPUT_DIR = process.env["IOS_OUTPUT_DIR"] || join5(process.cwd(), "source", "ios-backup", "line");
if (!PASSWORD) {
...
try {
  const result2 = await extractAndParseLineHistory(BACKUP_ROOT, UDID, PASSWORD, OUTPUT_DIR, (stage, current, total, message) => {
    console.log(`[${stage}] ${current}/${total}: ${message}`);
  });
  console.log(`
[cli] Done!`);
  console.log(`  Extracted: ${result2.extracted.lineFiles.length} LINE files`);
  console.log(`  Databases: ${result2.extracted.databases.length}`);
  console.log(`  Parsed chats: ${result2.parsed.chats.length}`);
  console.log(`  Total messages: ${Array.from(result2.parsed.messages.values()).reduce((a, b) => a + b.length, 0)}`);
} catch (e) {
