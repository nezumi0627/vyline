...
function extractException(struct: DecodedStruct): LineApiError | null {
  ...
  const msgStr = Buffer.isBuffer(message) ? message.toString("utf8") : String(message ?? "unknown error");
...
export class VylineRequestClient {
  ...
  getHeader(overrideMethod = "POST"): Record<string, string> {
    ...
    if (overrideMethod && overrideMethod !== "POST") header["x-http-method-override"] = overrideMethod;
    return header;
  }
