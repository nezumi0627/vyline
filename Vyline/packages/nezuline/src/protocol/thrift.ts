...
function writeCompactValue(
  ...
  switch (type) {
    ...
    case TType.I32:
      ...
      break;
    case TType.I64:
      writeVarint64(w, zigzag64(typeof value === "bigint" ? value : BigInt(Math.trunc(Number(value)))));
      break;
    case TType.DOUBLE:
    ...
    case TType.MAP: {
      ...
      w.byte(
        ((ttypeToCompact(map.keyType) << 4) | ttypeToCompact(map.valType)) &
          0xff,
      );
...
function writeCompactCollectionHeader(
  w: GrowableWriter,
  size: number,
  elemType: number,
): void {
