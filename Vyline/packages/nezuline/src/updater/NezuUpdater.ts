...
function logOf(opts: NezuUpdaterOptions) {
...
export class NezuUpdater {
  #opts: NezuUpdaterOptions;
  #dataDir: string;
  #profile: DesktopProfile | null = null;
  #watch: WatchHandle | null = null;

  constructor(options: NezuUpdaterOptions = {}) {
    this.#opts = {
    ...
    this.#dataDir = defaultNezuDataDir(options.dataDir);
  }
  ...
  async detect(): Promise<DetectResult> {
    ...
    if (process.platform !== "win32") {
      ...
      log.warn("[NezuUpdater] non-Windows — using cached/fallback Desktop profile");
      return {
    ...
    if (!installed) {
      this.#profile = cached ?? loadFallbackProfile();
      log.warn("[NezuUpdater] LINE Desktop not found — using cached/fallback");
      return {
        profile: this.#profile,
        fromCache: Boolean(cached),
        refreshed: false,
    ...
    if (
      ...
      this.#profile = cached;
      log.info(
        `[NezuUpdater] cache hit Desktop ${cached.identity.appVersion}`,
      );
      return {
        profile: cached,
        fromCache: true,
  ...
  async refresh(): Promise<DesktopProfile> {
    ...
    const systemVersion =
      normalizeOsFromUpdateLog(osHint.osVersion) ?? detectSystemVersion();
    const ntSuffix = detectNtSuffix(
      Number.parseInt(systemVersion.split(".")[2] ?? "0", 10),
    );
    ...
    log.info(
      `[NezuUpdater] refreshed Desktop profile ${identity.appVersion} via ${method}`,
    );
    log.debug?.(
      `[NezuUpdater] UA=${identity.userAgent} XLA=${identity.xLineApplication}`,
    );
    return profile;
  }
  ...
  async #fetchCdnLatest(): Promise<string | null> {
    const res = await fetch(UPDATE_INFO_URL, {
      headers: { "user-agent": "NezuUpdater/0.1" },
    });
...
export function createNezuUpdater(options?: NezuUpdaterOptions): NezuUpdater {
  return new NezuUpdater(options);
}
