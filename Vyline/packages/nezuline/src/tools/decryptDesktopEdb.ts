...
import { existsSync, mkdirSync, readFileSync, writeFileSync, openSync, readSync, closeSync, fstatSync } from "node:fs";
...
function findMainEdb(): string | null {
  ...
  const files = readdirSync(dir)
    .filter((f) => f.endsWith(".edb") && !f.startsWith("album_") && !f.startsWith("chatStats_") && !f.startsWith("keep_"))
    .map((f) => {
      const p = join(dir, f);
...
async function main() {
  ...
  const outDir =
    arg("out-dir") ??
    join(process.cwd().includes("nezuline") ? "../../../backend/data" : "Vyline/backend/data", "edb-export");
  // resolve relative to repo
  const dataDir = join(
    import.meta.dirname ?? ".",
    "../../../../backend/data/edb-export",
  );
  ...
  const plain = decryptWxSqlite3File(
    file,
    tryPassphraseVariants(hit.passphrase)[0]!,
    { lineQuirk: hit.quirk, pageSize: hit.pageSize },
  );
