 ...
 * 使い方 (リポジトリルート):
 *   bun run nezu:delta
 *   bun Vyline/packages/nezuline/src/tools/reportDesktopDelta.ts
 *
 * 出力:
...
function resolveDataDir(): string {
  if (process.env["NEZU_DATA_DIR"]) return process.env["NEZU_DATA_DIR"];
  if (process.env["VYLINE_DATA_DIR"]) {
    return join(process.env["VYLINE_DATA_DIR"], "nezuline");
  }
  // backend が使う data/nezuline を優先、なければパッケージ .cache
  const backendNezu = join(REPO_ROOT, "Vyline", "backend", "data", "nezuline");
  if (existsSync(profileJsonPath(backendNezu))) return backendNezu;
  return defaultNezuDataDir();
}
...
export function buildDesktopDeltaReport(opts?: {
  ...
  console.info(`[nezu:delta] lineRoot=${lineRoot}`);
  console.info(`[nezu:delta] dataDir=${dataDir}`);
  ...
  const profileSource: DesktopDeltaReport["cached"]["source"] = cached
    ? "cache"
    : "fallback";
  ...
  if (installed) {
    ...
    console.info(
      `[nezu:delta] installed=${installed.version} sha=${installed.exeSha256.slice(0, 12)}…`,
    );
  } else {
    notes.push("LINE Desktop インストールが検出できなかった (Windows + %LOCALAPPDATA%\\LINE 想定)");
    console.warn("[nezu:delta] LINE Desktop not found");
  }
  ...
  const shaChanged = Boolean(
    installed &&
      profile.source.exeSha256 &&
      installed.exeSha256 !== profile.source.exeSha256,
  );
  ...
  if (versionChanged) {
    ...
    notes.push(
      `appVersion は同じ (${curVer}) だが exeSha256 が変化 — 再抽出を推奨`,
    );
  ...
  if (!suggestAll) {
    notes.push(
      "差分なしのため high priority モジュールのみ列挙 (全件は bun run nezu:delta -- --all)",
    );
  }
...
async function main(): Promise<void> {
  ...
  console.info("[nezu:delta] building Desktop delta report…");
  if (allModules) console.info("[nezu:delta] --all: listing every feature module");
  const report = buildDesktopDeltaReport({ allModules });
  const out = writeReports(report);
  console.info(`[nezu:delta] status=${report.delta.status}`);
  console.info(
    `[nezu:delta] ${report.delta.previousVersion ?? "?"} → ${report.delta.currentVersion ?? "?"}`,
  );
  console.info(`[nezu:delta] wrote ${out.md}`);
  console.info(`[nezu:delta] wrote ${out.json}`);
  console.info(
    `[nezu:delta] suggested modules: ${report.suggestedModules.map((m) => m.id).join(", ")}`,
  );
}
