...
import type { BaseClient } from "@vyline/nezuline/stack/base";
...
async function listLocalSelfKeys(base: BaseClient): Promise<Array<{ keyId: number; privKey: string; pubKey: string }>> {
  const mid = base.profile?.mid;
  const ids = new Set<number>();
  if (mid) {
    const midRaw = await base.storage.get(`e2eeKeys:${mid}`);
...
export async function ensureGroupKeyById(
  ...
  try {
    ...
    if (Number(shared.groupKeyId) !== groupKeyId) {
      throw new Error(
        `group key ${groupKeyId} unavailable (last is ${shared.groupKeyId})`,
      );
    }
  }
...
export async function prepareGroupKeysForMessages(
  ...
  const g = globalThis as unknown as { __nezuGroupKeyWipe?: Set<string> };
  if (!g.__nezuGroupKeyWipe) {
    g.__nezuGroupKeyWipe = new Set();
  }
  const wiped = g.__nezuGroupKeyWipe;
  ...
  const results = await Promise.all(
    [...ids].map(async (id) => {
      try {
        ...
        asBase(client).log("nezu:group-e2ee", {
...
export function patchGroupKeyLookup(client: AnyClient): void {
  ...
  if (e2ee.__nezuGroupKeyLookupPatchedV3) return;

  if (typeof e2ee.tryRegisterE2EEGroupKey === "function" && !e2ee.__nezuTryRegisterBlocked) {
    e2ee.__nezuOriginalTryRegisterE2EEGroupKey = e2ee.tryRegisterE2EEGroupKey.bind(e2ee);
    ...
    e2ee.__nezuTryRegisterBlocked = true;
  }

  const original = e2ee.getE2EELocalPublicKey.bind(e2ee);

  e2ee.getE2EELocalPublicKey = async function (
    mid: string,
    keyId?: string | number,
  ) {
    ...
    if (isGroup) {
      ...
      try {
        ...
        const originalRegister = e2ee.__nezuOriginalTryRegisterE2EEGroupKey as
  ...
  e2ee.__nezuGroupKeyLookupPatchedV3 = true;
  base.log("nezu:group-e2ee", { phase: "lookup-patched-v3" });
}
...
export async function recreateE2EEGroupKey(
  ...
  // biome-ignore lint/suspicious/noExplicitAny: protocol e2ee
  const e2ee = base.e2ee as any;
  const original = e2ee.__nezuOriginalTryRegisterE2EEGroupKey as
  ...
  await base.storage.delete(`e2eeGroupKeys:${chatMid}`).catch(() => undefined);
  base.log("nezu:group-e2ee", { phase: "recreate-start", chatMid });
