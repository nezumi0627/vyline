...
async function registerFreshSenderKey(
  base: BaseClient,
  mid: string,
): Promise<LocalKey> {
  ...
  base.log("nezu:e2ee", { phase: "fresh-sender-key", keyId });
  return keyData;
}
...
export async function ensureValidE2EEIdentity(
  ...
  const serverLatestKeyId =
    serverKeyIds.length > 0 ? Math.max(...serverKeyIds) : null;
  ...
  const hasLatestSender =
    serverLatestKeyId !== null &&
    matched.some((m) => m.keyId === serverLatestKeyId);

  // 強制、または明示許可時のみ新規登録。既定ではスマホ公式を壊さない。
  const mayRegister =
    Boolean(opts.forceNewSenderKey) || Boolean(opts.allowRegisterNewKey);
