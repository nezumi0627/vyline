...
import { decryptKeyChain, generateSharedSecret, getSHA256Sum, verifyE2EEKeyPair, xorHalves } from "../e2ee/primitives.js";
...
export function patchDesktopLogin(base: BaseClient): void {
  ...
  login.qrCodeLoginV2ForSecure = (
    ...
    const model =
      !modelName || modelName === "evex-device" ? pc.modelName : modelName;
    const system =
      !systemName || systemName === "vyline-device" ? pc.systemName : systemName;
    base.log("nezu:qr", {
      systemName: system,
      modelName: model,
  ...
  login.getRSAKeyInfo = async (provider = 0) => {
    base.log("nezu:rsa", { path: DESKTOP_TALK_RSA, method: "getRSAKeyInfo" });
  ...
  login.loginV2 = async (
    ...
    // トランスポートを使う (ensureE2EE.ts / NezuClient.ts 側は既に生の
    // フィールド番号でレスポンスを読んでいる)。
    if (methodName === "loginZ") {
      return await base.request.request(fields as never, methodName, 3, "LoginResult", DESKTOP_AUTH_RS);
    }
  ...
  login.confirmE2EELogin = async (verifier: string, deviceSecret: string) => {
    base.log("nezu:confirmE2EE", { path: DESKTOP_AUTH_RS });
