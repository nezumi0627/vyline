...
import { defaultCallFromEnvInfo } from "@vyline/nezuline/stack/call";
...
/** Desktop NezuProfile から Planet UA を合成（LINE.exe 追従） */
...
export function buildPlanetUserAgent(
  ...
  if (
    desktopProfile &&
    process.env["VYLINE_CALL_DESKTOP_WIRE"] === "1"
  ) {
    return buildDesktopPlanetUserAgent(desktopProfile);
  }

  if (
    (details.device === "DESKTOPWIN" || details.device === "DESKTOPMAC") &&
    desktopProfile
  ) {
