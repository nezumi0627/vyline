...
`;
  ...
  for (const s of samples) {
    identity =
      parseRuntimeApplicationHeader(s) ??
      parseRuntimeUserAgent(s) ??
      identity;
    if (identity) break;
  }
...
export function extractFromExe(
  exePath: string,
  appVersion: string,
): ExtractResult {
  const notes: string[] = [];
  ...
  notes.push(`pe ascii strings scanned (partial): ${strings.length}, desktop-like: ${samples.length}`);
