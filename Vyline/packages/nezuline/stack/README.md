...
`@vyline/nezuline` 内部の LINE プロトコル RPC スタックです。Desktop 準拠の Legy トランスポートは `RequestClient` + `patchDesktopTransport` / `patchDesktopLogin` で実装しています。

- Talk: `/S4` (Compact Thrift)
- Auth: `/api/v3p/rs`, getRSAKeyInfo: `/api/v3/TalkService.do`
- E2EE: `letterSealing`（nezuline）+ stack `e2ee/`
- OBS: stack `obs/`
