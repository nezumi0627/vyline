...
```
NezuUpdater ──detect/refresh──► DesktopProfile (UA / X-Line-Application / hosts)
        │
        ▼
NezuClient (@vyline/nezuline/stack + Desktop patches)
...
| 更新追従 | `src/updater/NezuUpdater.ts` |
| ログイン / E2EE | `src/login/*` |
| クライアント Facade | `src/client/NezuClient.ts` |
...
Backend は `Vyline/backend/src/nezu/profileBridge.ts` 経由で `NezuUpdater` を起動時に初期化する。
...
3. **ヘッダーだけ追従したい**  
   - `NezuUpdater.refresh()` または backend `POST /debug/nezu/refresh`
...
依存: 内部 `stack/`（Thrift RPC）、`@vyline/line-types`（型のみ vendored）。backend は workspace 経由で `@vyline/nezuline` を利用。
...
- Updater: `createNezuUpdater`, `detectInstalledDesktop`
